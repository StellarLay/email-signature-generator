...
const fields = [
  { name: "firstname", label: "Имя", placeholder: "Анна", icon: UserRound, autoComplete: "given-name" },
  { name: "lastname", label: "Фамилия", placeholder: "Смирнова", icon: UserRound, autoComplete: "family-name" },
  { name: "position", label: "Должность", placeholder: "Head of Communications", icon: BriefcaseBusiness, autoComplete: "organization-title", wide: true },
  { name: "phone", label: "Телефон", placeholder: "+7 999 123-45-67", icon: Phone, autoComplete: "tel", inputMode: "tel" },
  { name: "email", label: "Рабочая почта", placeholder: "name@reputation.house", icon: AtSign, autoComplete: "email", inputMode: "email", type: "email" },
];
...
const PhotoField = ({ data, onChange }) => {
  ...
  return (
    <Field.Root gridColumn="1 / -1" invalid={Boolean(error)}>
      <Field.Label color="#374139" fontSize="sm" fontWeight="700">Фотография</Field.Label>
      ...
      <Box position="relative" w="full">
        ...
        <Input
          ...
          placeholder="Ссылка Google Drive или прямая HTTPS-ссылка"
          autoComplete="url"
          inputMode="url"
          h="12"
          ps="11"
      ...
      <Flex align="center" w="full" gap="3" my="1">
        ...
        <Text color="#929c91" fontSize="xs" fontWeight="700" textTransform="uppercase" letterSpacing="0.1em">или</Text>
        <Box h="1px" flex="1" bg="#e2e7df" />
      </Flex>
        ...
        {data.photoFileName ? (
          <Flex align="center" justify="space-between" gap="3">
            <Flex align="center" gap="3" minW="0">
              ...
              <Box minW="0">
                ...
                <Badge mt="1" bg="#dfeadb" color="#536c4d" borderRadius="full" px="2" fontSize="2xs">Файл загружен</Badge>
              </Box>
            </Flex>
            <Button type="button" size="xs" variant="ghost" color="#687367" onClick={clearFile} aria-label="Удалить фотографию">
              <Icon as={X} boxSize="4" />
            </Button>
          </Flex>
        ) : (
          <Flex align={{ base: "flex-start", sm: "center" }} justify="space-between" gap="4" direction={{ base: "column", sm: "row" }}>
            <Flex align="center" gap="3">
              ...
              <Box>
                <Text fontSize="sm" fontWeight="700" color="#3f4940">Перетащите фотографию сюда</Text>
                <Text mt="0.5" fontSize="xs" color="#828c81">JPG, PNG или WebP до 2 МБ</Text>
              </Box>
            </Flex>
      ...
      <Field.HelperText color="#7c867c" fontSize="xs" lineHeight="1.5">
        Используется только один вариант: ссылка или загруженный файл. Для Google Drive включите доступ «Все, у кого есть ссылка».
      </Field.HelperText>
...
const SignatureForm = ({ data, onChange }) => {
  const handleChange = (event) => {
    ...
    onChange((currentData) => ({ ...currentData, [name]: value }));
  };
  ...
  return (
    <Box as="section" bg="rgba(255,255,255,.88)" border="1px solid" borderColor="#e1e7de" borderRadius="3xl" p={{ base: "5", md: "7" }} boxShadow="0 22px 70px rgba(47, 65, 46, 0.08)" backdropFilter="blur(14px)">
      <Flex align="center" justify="space-between" gap="4" mb="6">
        <Box>
          <Text color="#718070" fontSize="xs" textTransform="uppercase" letterSpacing="0.14em" fontWeight="800">Шаг 1</Text>
          <Heading as="h2" mt="1" fontSize="2xl" letterSpacing="-0.025em">Данные сотрудника</Heading>
        </Box>
